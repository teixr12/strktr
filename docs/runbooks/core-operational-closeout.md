...
```bash
npm run ops:core-certification
```

Optional overrides:
- `CORE_CERT_BASE_URL`
- `CORE_CERT_EXPECT_BRANCH`
- `CORE_CERT_EXPECT_VERSION`
- `CORE_CERT_AUTH_STRICT_STATUS=pass`
- `CORE_CERT_HEALTH_JSON`
- `CORE_CERT_RELEASE_JSON`

The readiness check fails if:
- release traceability is inconsistent on the live alias
- auth strict is not marked stable
- one of the rollback drill reports is missing
...
4. `npm run ops:core-certification`
5. `node scripts/generate-core-operational-closeout.mjs`
