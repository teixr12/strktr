...
import { isPublicApiEnabled } from '@/lib/public-api/feature'
import { isPublicApiV1EnabledForOrg } from '@/server/feature-flags/wave2-canary'

export function requirePublicApiEnabled(request: Request): Response | null {
...
export function withPublicApiAuth(
  ...
  return async (request: Request) => {
    const gate = requirePublicApiEnabled(request)
    if (gate) return gate
    const authHandler = withApiAuth(permission, async (innerRequest, ctx) => {
      if (!isPublicApiV1EnabledForOrg(ctx.orgId)) {
        return fail(
          innerRequest,
          {
            code: API_ERROR_CODES.NOT_FOUND,
