import type { UiDataColumn } from '@/shared/types/ui'
...
export function DataTable<T>({ columns, rows, emptyMessage = 'Sem dados' }: DataTableProps<T>) {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
          <thead className="bg-gray-50/80 dark:bg-gray-900/50">
            <tr>
              {columns.map((column) => (
                <th key={column.key} className={`px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 ${column.className || ''}`}>
                  {column.header}
                </th>
              ))}
            </tr>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row, rowIndex) => (
                <tr key={rowIndex} className="hover:bg-gray-50/80 dark:hover:bg-gray-900/60">
                  {columns.map((column) => (
                    <td key={column.key} className={`px-4 py-3 text-sm text-gray-700 dark:text-gray-300 ${column.className || ''}`}>
                      {column.cell(row)}
                    </td>
                  ))}
                </tr>
