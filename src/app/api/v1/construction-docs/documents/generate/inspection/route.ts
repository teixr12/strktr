...
import { generateInspectionDocument } from '@/server/services/construction-docs/generate-service'
...
export const POST = withConstructionDocsAuth('can_manage_projects', async (request, { supabase, orgId, user }) => {
  ...
  try {
    const document = await generateInspectionDocument({
      supabase,
      orgId,
      userId: user.id,
      ...
      prompt:
        payload.prompt?.trim() ||
        `Gerar relatório de vistoria de obra para projeto ${payload.project_id}.`,
    ...
    await appendConstructionAudit({
      ...
      orgId,
      actorUserId: user.id,
      eventType: 'document_generated_inspection',
      projectId: payload.project_id,
    ...
    return fail(
      ...
      {
        code: API_ERROR_CODES.DB_ERROR,
        message: error instanceof Error ? error.message : 'Falha ao gerar documento de inspeção',
      },
      500
    )
  }
