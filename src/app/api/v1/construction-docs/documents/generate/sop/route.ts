...
import { generateSopDocument } from '@/server/services/construction-docs/generate-service'
...
export const POST = withConstructionDocsAuth('can_manage_projects', async (request, { supabase, orgId, user }) => {
  ...
  try {
    const document = await generateSopDocument({
      supabase,
      orgId,
      userId: user.id,
      ...
      prompt:
        payload.prompt?.trim() ||
        `Gerar SOP técnico para projeto ${payload.project_id} com etapas claras.`,
    ...
    await appendConstructionAudit({
      ...
      orgId,
      actorUserId: user.id,
      eventType: 'document_generated_sop',
      projectId: payload.project_id,
    ...
    return fail(
      request,
      {
        ...
        message: error instanceof Error ? error.message : 'Falha ao gerar SOP',
      },
      500
    )
  }
