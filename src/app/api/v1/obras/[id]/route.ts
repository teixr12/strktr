...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error: dbError } = await supabase.from('obras').select('*').eq('id', id).single()
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error: dbError } = await supabase.from('obras').update(body).eq('id', id).select().single()
