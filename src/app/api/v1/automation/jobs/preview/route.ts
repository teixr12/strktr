...
import { runAutomationPreviewSchema } from '@/shared/schemas/roadmap-automation'
import { previewAutomation } from '@/server/services/automation/automation-service'
...
export async function POST(request: Request) {
  ...
  const parsed = runAutomationPreviewSchema.safeParse(await request.json().catch(() => null))
  if (!parsed.success) {
    return fail(
      request,
      {
  ...
  const preview = await previewAutomation(supabase, {
    orgId,
    userId: user.id,
  ...
  return ok(request, preview, { flag: 'NEXT_PUBLIC_FF_SEMI_AUTOMATION' })
}
